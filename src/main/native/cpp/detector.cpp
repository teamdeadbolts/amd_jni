...
Detector::Detector(const std::string &model_path, int numClasses, int modelVersion, int deviceMask) 
...
//   std::cout.rdbuf(logFile.rdbuf());
  ...
  for (size_t i = 0; i < numInputNodes; i++) {
    Ort::AllocatedStringPtr nameAlloc = session->GetInputNameAllocated(i, allocator);
    const char* namePtr = nameAlloc.get();
    
    inputNamesAlloc.push_back(std::move(nameAlloc));
    inputNames.push_back(inputNamesAlloc.back().get());  // Get pointer from stored allocation
  ...
  for (size_t i = 0; i < numOutputNodes; i++) {
    Ort::AllocatedStringPtr nameAlloc = session->GetOutputNameAllocated(i, allocator);
    const char* namePtr = nameAlloc.get();
    
    outputNamesAlloc.push_back(std::move(nameAlloc));
    outputNames.push_back(outputNamesAlloc.back().get());  // Get pointer from stored allocation
...
std::vector<DetectionBox> Detector::detect(cv::Mat& image, float nmsThresh, float boxThresh) {
  // Preprocess image
  cv::Mat inputBlob;
  ...
  if (elementType == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16) {
    ...
    return parseOutputAndNMS(outputTensors[0], image.cols, image.rows, inputW, inputH, nmsThresh, boxThresh);
...
std::vector<DetectionBox> Detector::parseOutputAndNMS(Ort::Value& output, int origW, int origH,
  ...
  if (modelVersion == 0) { // YOLOv5
    ...
    detections = parseYOLOv5Output(output, origW, origH, inputW, inputH, boxThresh);
  } else { // YOLOv8/v11
    std::cout << "Using YOLOv8/v11 parser..." << std::endl;
    detections = parseYOLOv8Output(output, origW, origH, inputW, inputH, boxThresh);
  }
...
std::vector<DetectionBox> Detector::parseYOLOv5Output(Ort::Value& output, int origW, int origH,
                                            int inputW, int inputH, float boxThresh) {
...
std::vector<DetectionBox> Detector::parseYOLOv8Output(Ort::Value& output, int origW, int origH,
                                             int inputW, int inputH, float boxThresh) {
  std::vector<DetectionBox> boxes;
  float* outputData = output.GetTensorMutableData<float>();
  auto shape = output.GetTensorTypeAndShapeInfo().GetShape();
...
std::vector<DetectionBox> Detector::applyNMS(std::vector<DetectionBox>& boxes, float nmsThresh) {
    // Sort by confidence
